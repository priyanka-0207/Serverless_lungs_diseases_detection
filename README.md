...
See `docs/ARCHITECTURE.md` for the full build, phase by phase.
...
See `notebooks/lungs_disease_detection.ipynb` for training and export.
...
The train/validation split is done before any oversampling, so the minority-class oversampling applies to training data only and does not leak into validation. A confusion matrix and full classification report are in `docs/RESULTS.md`.

### Example predictions

One example per class from the live app. Percentages are the model's per-image confidence (softmax) for that prediction, not overall accuracy. Overall validation accuracy is 72% (macro F1 0.72); see `docs/RESULTS.md`.
...
| App home | CloudWatch |
|---|---|
| ![Landing page and AWS pipeline overview](assets/home.png) | ![CloudWatch dashboard: invocations, duration, errors](assets/cloudwatch.png) |

## To Use

The training notebook runs in Google Colab or any GPU environment with PyTorch and torchvision. The AWS deployment is documented step by step in `docs/ARCHITECTURE.md`.
